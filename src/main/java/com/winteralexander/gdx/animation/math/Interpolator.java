...
public class Interpolator {
	...
	public static float quinticAngle(float a, float b, float c, float d, float e, float f,
	                                 float t) {
	...
	public static float bezier(float t, float x1, float x2, float x3, float x4) {
		...
		return (-temp * t + 3 * temp - 3 * t + 1) * x1 + (3 * t * temp1 - 6 * temp1 + 3 * t) * x2 +
				(-3 * temp2 * t + 3 * temp2) * x3 + t * t * t * x4;
	}
	...
	public static float solveCubic(float a, float b, float c, float d) {
		...
		//DEBUG
		System.out.println("WARNING: No cubic solution found for " + a + ", " + b + ", " + c + ", "
				+ d);
		return -1;
	}
